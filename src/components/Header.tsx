...
export default function Header({ title, backHref }: { title?: string; backHref?: string }) {
  ...
  const { currentUser, role, isAnonymous, logout } = useAuth();
                      ...
                      <>
                        ...
                        {role === 'patient' && (
                          <>
                            ...
                            <a href="/patient" onClick={() => setOpen(false)} className="flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-teal-700 transition-colors">
                              <Activity size={15} className="text-slate-400" /> Patient Intake Kiosk
                            </a>
